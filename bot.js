...
const devs = ['573644720081731604'];
const db = require('quick.db');
const premium = ['573644720081731604']
...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['$vsnhelp','Volcano System'];
    var i = -1;
    var j = 0;
...
${prefix}bc / رسالة جماعيه مع
${prefix}admin / رساله برود كاست بدون منشن
...
client.on('message', message => {
    if(!message.channel.guild) return;
let args = message.content.split(' ').slice(1).join(' ');
if (message.content.startsWith('$vadmin')){
if(!message.author.id === '502509585370841098') return;
...
client.on("message", (message) => {
  ...
  if (message.content.toLowerCase().startsWith(prefix + `help`)) {
    ...
    .setColor(0xCF40FA)
    .setDescription(`Hello! I'm Volcano System, the Discord bot for super cool support ticket stuff and more! Here are my commands:`)
    .addField(`Tickets`, `[${prefix}new]() > Opens up a new ticket and tags the Support Team\n[${prefix}close]() > Closes a ticket that has been resolved or been opened by accident`)
    .addField(`Other`, `[${prefix}help]() > Shows you this help menu your reading\n[${prefix}ping]() > Pings the bot to see how long it takes to react\n[${prefix}about]() > Tells you all about Volcano System`)
...
if (message.content.toLowerCase().startsWith(prefix + `close`)) {
    ...
    .then((m) => {
      ...
      .then((collected) => {
        ...
        .catch(() => {
          m.edit('تم اغلاق التيكت بسبب التاخر في الرد').then(m2 => {
              m2.delete();
          }, 3000);
        });
    });
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			...
			try {//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
				...
				try {//by ,$ ReBeL ء , ??#4777 'CODES SERVER'
			        ...
			        .setDescription(`**الرجاء منك اختيار رقم المقطع,** :
...
client.on("guildMemberAdd", member => {
  member.createDM().then(function (channel) {
  return channel.send(`Welcome To N J C Community
